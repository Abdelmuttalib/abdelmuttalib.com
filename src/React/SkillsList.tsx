...
const CategoryIcons = {
  "Web Development": (
    <svg
      ...
      viewBox="0 0 24 24"
      fill="currentColor"
      className="text-accent h-6 w-6 opacity-70"
    >
...
const SkillsList = () => {
  ...
  return (
    <div className="pt-3 text-left md:pt-9">
      <h3 className="text-foreground text-3xl font-semibold md:mb-6 md:text-4xl">
        What I do?
      </h3>
        ...
        {Object.entries(skills).map(([category, items]) => (
          <li key={category} className="w-full">
            <div
              ...
              className="hover:bg-opacity-80 bg-card border-border w-full cursor-pointer overflow-hidden rounded-2xl border text-left transition-all md:w-[400px]"
            >
              <div className="flex items-center gap-3 p-4">
                {CategoryIcons[category]}
                <div className="flex flex-grow items-center justify-between gap-2">
                  <div className="max-w-[200px] min-w-0 overflow-hidden md:max-w-none">
                    <span className="text-foreground block truncate text-lg">
                      {category}
                    </span>
                  </div>
                  <svg
                    ...
                    viewBox="0 0 24 24"
                    fill="currentColor"
                    className={`text-foreground h-6 w-6 flex-shrink-0 transform transition-transform ${
