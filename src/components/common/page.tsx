...
export function FormActions({
  ...
  return (
    <Inline
      className={cn(
        "mt-6 flex flex-col-reverse items-stretch justify-end sm:flex-row",
        className,
      )}
      gap="compact"
      {...props}
...
export function FormLayout({
  ...
  return (
    <div
      className={cn(
        "grid grid-cols-1 gap-x-10 gap-y-6 md:grid-cols-2 xl:grid-cols-3",
        className,
      )}
      {...props}
    />
...
export function FormSectionHeader({
  ...
  return (
    ...
    >
      {icon && (
        <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-primary/15 text-primary [&>svg]:h-5 [&>svg]:w-5">
          {icon}
        </div>
      )}
...
export function PageGridFull({ ...props }: GridItemProps & DivPropsType) {
  return <GridItem col="col-span-12" {...props} />;
}
